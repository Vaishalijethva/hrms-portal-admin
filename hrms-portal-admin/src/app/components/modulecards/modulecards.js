...
export default function ModuleCards({activeCard, cardName, onClick, title, imgClass, number }){
    ...
    return(
        <>
        {/* active:text-[#323232] [.active]:border-[#7081CB]   2xl:w-[281px] lg:w-[281px] md:w-[180px] w-[150px]   ${isOpen ? "w-[242px]" : "w-[277px]"}*/}
        <div className={`module-card group ${activeCard === cardName ? "active" : ""}  2xl:flex lg:flex md:flex flex 2xl:items-center lg:items-center md:items-center items-center 2xl:justify-start lg:justify-start md:justify-start justify-start 2xl:p-[20px] lg:p-[20px] md:p-[20px] sm:p-[10px] bg-white rounded-[10px] border border-[#FFF] cursor-pointer transition-transform duration-75 ease-in-out`}  onClick={() => onClick(cardName)}>
       
            <Link href="#" className="link-absolute focus-visible:outline-none" title={title}></Link>
            {/* group-[.active]:bg-[#7081CB] */}
            <div className={`module-up-img-card ${imgClass} 2xl:w-[60px] lg:w-[60px] md:w-[60px] w-[60px] 2xl:h-[60px] lg:h-[60px] md:h-[60px] h-[60px] rounded-[100%] 2xl:p-[10px] lg:p-[10px] flex items-center justify-center text-center mr-[15px] bg-[#E6F4FF] relative before:content[''] before:absolute before:left-0 before:right-0 before:w-[25px] before:h-[25px] before:bg-no-repeat before:m-auto before:transition-transform before:duration-75 before:ease-in-out transition-transform duration-75 ease-in-out ` } ></div>
